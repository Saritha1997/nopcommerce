...
class Add_Customer_Page:
    ...
    # locator from add customer page
    ...
    newsletter_cusrole_list_xpath = "//div[@role='listbox']"
    ...
    cusrole_registers_xpath = "//li[contains(text(),'Registered')]"
    ...
    def select_customer_role(self,role):
        ...
        time.sleep(3)
        if role == "Guests":
            self.driver.find_element(By.XPATH, self.cusrole_registers_xpath).click()
            time.sleep(3)
            cusrole_field.click()
    ...
    def select_manager_of_vendor(self, value):
        drp_dwn = Select(self.driver.find_element(By.XPATH, self.drpdwn_mngrofvendor_id))
